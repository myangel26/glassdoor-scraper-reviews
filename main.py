...
def scrape(field, review, author):
    ...
    def scrape_date(review):
        ...
        date = review.find_element(By.CSS_SELECTOR, ".middle.common__EiReviewDetailsStyle__newGrey").text.split('-')[0]
        # time_index = date.find(':') - 3
        # res = date[:time_index]
        posps = dt.datetime.strptime(str(date).replace(',', '').strip(), '%b %d %Y').strftime('%d/%m/%Y')
        return posps

    def scrape_emp_title(review):
        if 'Anonymous Employee' not in review.text:
            try:
                res = review.find_element(By.CSS_SELECTOR, ".middle.common__EiReviewDetailsStyle__newGrey").text.split(' - ')[1]
            except Exception:
    ...
    def scrape_location(review):
        if 'in' in review.text:
            try:
                res = review.find_element(By.XPATH, './/span[contains(@class, "common__EiReviewDetailsStyle__newUiJobLine")]/span/span[contains(@class, "middle")]/span').text
            except Exception:
    ...
    def scrape_status(review):
        try:
            ...
            res = review.find_element(By.XPATH, ".//span[contains(@class,'eg4psks0')]").text.split(',')[0]
        except Exception:
    ...
    def scrape_experience(review):
        try:
            res = ''
            # res = author.text.split('-')[0]
            res = review.find_element(By.XPATH, ".//span[contains(@class,'eg4psks0')]").text.split(',')[1].strip('"')
    ...
    def scrape_rev_title(review):
        try:
            res = ''
            res = review.find_element(By.XPATH, ".//h2[contains(@class,'el6ke055')]/a[contains(@class,'reviewLink')]").text.strip('"')
    ...
    def scrape_helpful(review):
        try:
            ...
            res = review.find_element(By.XPATH, ".//div[contains(@class,'common__EiReviewDetailsStyle__socialHelpfulcontainer')]").text.split(' ')[0]
    ...
    def expand_show_more(section):
        try:
            more_link = section.find_element(By.XPATH, ".//div[contains(@class,'v2__EIReviewDetailsV2__continueReading')]")
            more_link.click()
        except Exception:
            pass
    ...
    def scrape_pros(review):
        try:
            ...
            res = review.find_element(By.XPATH, ".//span[@data-test='pros']").text.strip('"')
            res = replace_character(res)
        except Exception:
            res = np.nan
        return res
    ...
    def scrape_cons(review):
        try:
            ...
            res = review.find_element(By.XPATH, ".//span[@data-test='cons']").text.strip('"')
            res = replace_character(res)
        except Exception:
            res = np.nan
        return res
    ...
    def scrape_advice(review):
        try:
            ...
            # else:
            #     res = np.nan
            res = review.find_element(By.XPATH, ".//span[@data-test='advice-management']").text.strip('"')
            res = replace_character(res)
        except Exception:
            res = np.nan
            res = 'N/A'
    ...
    def scrape_overall_rating(review):
        try:
            ...
            res = review.find_element(By.CLASS_NAME, 'ratingNumber').text.strip('')
        except Exception:
            res = np.nan
        return res
    ...
    def _scrape_subrating(ratingName):
        try:
            # ratings = review.find_element_by_class_name('gdStars')
            subratings = review.find_element(By.CLASS_NAME, 'tooltipContainer').find_element(By.TAG_NAME, 'ul')
            res = 'NULL'
            for x in subratings.find_elements(By.TAG_NAME, 'li'):
                # name_title = x.find_element_by_class_name('minor').get_attribute('innerHTML').strip()
                name_title = x.find_element(By.CSS_SELECTOR, 'div:first-child').get_attribute('innerHTML').strip()
                if ratingName == name_title:
                    # res = x.find_element_by_class_name('gdBars').get_attribute('title')
                    css_bg = x.find_element(By.CSS_SELECTOR,'div:nth-child(2)').value_of_css_property('background')
    ...
    def scrape_recommends(review):
        try:
            arr_recommends = review.find_element(By.CLASS_NAME, 'recommends')
            color_recommend = arr_recommends.find_element(By.CSS_SELECTOR, 'div:nth-child(1)').find_element(By.TAG_NAME, 'svg').value_of_css_property('color')
            # res = res.split('\n')
    ...
    def scrape_outlook(review):
        try:
            ...
            # return np.nan
            arr_recommends = review.find_element(By.CLASS_NAME, 'recommends')
            color_recommend = arr_recommends.find_element(By.CSS_SELECTOR, 'div:nth-child(3)').find_element(By.TAG_NAME, 'svg').value_of_css_property('color')
            res = check_color(color_recommend)
            return res
        except:
            return np.nan
    
    def scrape_company_response(review):
        try:
            res = review.find_element(By.XPATH, ".//div[contains(@class,'v2__EIReviewDetailsV2__newEmployerResponse')]")
            res = 'Yes'
        except Exception:
            # logger.warning('Failed to scrape company response')
            res = "No"
        return res
    ...
    def scrape_approve_ceo(review):
        try:
            ...
            # return np.nan
            arr_recommends = review.find_element(By.CLASS_NAME, 'recommends')
            color_recommend = arr_recommends.find_element(By.CSS_SELECTOR, 'div:nth-child(2)').find_element(By.TAG_NAME, 'svg').value_of_css_property('color')
            res = check_color(color_recommend)
            return res
        except:
            return np.nan
...
def extract_from_page():
    ...
    def is_featured(review):
        try:
            review.find_element('class name', 'featuredFlag')
            return True
    ...
    def extract_review(review):
        try:
            # author = review.find_element('class name','authorInfo')
            author = review.find_element(By.CLASS_NAME, "eg4psks0")
        except:
    ...
    reviews = browser.find_elements('class name', 'empReview')
    ...
    for review in reviews:
        if not is_featured(review):
            ...
            if data != None:
                ...
                pw_date = review.find_element(By.CSS_SELECTOR, ".middle.common__EiReviewDetailsStyle__newGrey").text.split('-')[0] = review.find_element(By.CSS_SELECTOR, ".middle.common__EiReviewDetailsStyle__newGrey").text.split('-')[0]
                pw_posps = dt.datetime.strptime(str(pw_date).replace(',', '').strip(), '%b %d %Y').strftime('%d/%m/%Y')
                # print(pd.to_datetime(pw_posps, format='%d/%m/%Y'))
                if args.max_date is not None:
                    if (pd.to_datetime(pw_posps, format='%d/%m/%Y') > args.max_date).bool():
                        logger.info('Date limit reached, ending process')
                        date_limit_reached[0] = True
                
                if args.min_date is not None:
                    if (pd.to_datetime(pw_posps, format='%d/%m/%Y') < args.min_date):
                        logger.info('Date limit reached, ending process')
                        date_limit_reached[0] = True
    ...
    # print('->>>>>>>>', pd.to_datetime("23 Apr 2023", format='%d %b %Y'))
    # print('=====================', pd.to_datetime(res['date']) > args.max_date)
    # print(args.max_date)
    # print(pd.to_datetime(res['date'], format='%d/%m/%Y') > args.max_date)
    # if args.max_date and \
    #     (pd.to_datetime(res['date'], format='%d/%m/%Y') > args.max_date) | \
    #         args.min_date and \
    #         (pd.to_datetime(res['date'], format='%d/%m/%Y') < args.min_date):
    #     logger.info('Date limit reached, ending process')
    #     date_limit_reached[0] = True

    # if args.max_date is not None:
    #     if (pd.to_datetime(res['date'], format='%d/%m/%Y') > args.max_date).bool():
    #         logger.info('Date limit reached, ending process')
    #         date_limit_reached[0] = True
    
    # if args.min_date is not None:
    #     if (pd.to_datetime(res['date'], format='%d/%m/%Y') < args.min_date):
    #         logger.info('Date limit reached, ending process')
    #         date_limit_reached[0] = True
    return res
...
def more_pages():
    try:
        current = browser.find_element('class name', 'selected')
        pages = browser.find_element('class name', 'pageContainer').text.split()
...
def go_to_next_page():
    ...
    next_ = browser.find_element('class name','nextButton')
...
def sign_in():
    ...
    btn_submit_with_email = browser.find_element('name', 'submit')
    ...
    btn_submit_signIn = browser.find_element('name', 'submit')
